...
def get_neo4j_retriever(graph, document_names,chat_mode_settings, score_threshold=CHAT_SEARCH_KWARG_SCORE_THRESHOLD):
    try:
        ...
        # Scale search_k based on number of documents to ensure we get results from all selected documents
        if document_names and chat_mode_settings["document_filter"]:
            num_documents = len(document_names)
            search_k = search_k * num_documents
            logging.info(f"Scaling search_k to {search_k} for {num_documents} selected documents")
